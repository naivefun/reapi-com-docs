...
## ReAPI CLI Command Overview

The ReAPI CLI provides a set of commands to interact with the platform directly from your terminal. Below are some useful commands for using the CLI:

- **General Help**: Use `npx @reapi/cli -h` to get help on all available commands.
- **Test Deployment Help**: Use `@reapi/cli test -h` for information on how to deploy and run tests.
- **Load Test Proxy Help**: Use `@reapi/cli server -h` for information on setting up a load test proxy.

Visit your **test deployment page** to copy related commands directly for ease of use.
